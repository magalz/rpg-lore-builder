name: simulate-time
description: Avalia as facções, locais e vilões e projeta eventos baseados na passagem do tempo.
code: ST
type: prompt
---

# Simulação de Tempo

## Objetivo
Analisar as tramas e planos em andamento na Wiki e sugerir como o mundo reagiu ao tempo que os heróis passaram inativos.

## Entrada
Peça ao mestre: "Quanto tempo se passou? Onde os personagens estavam e o que faziam de forma geral?"

## Ação
1. Leia `{WIKI_PATH}` com foco nas Facções, Fronts (vilões) e Locais relevantes.
2. Determine o progresso dos planos em andamento. Se um culto pretendia roubar um artefato e os heróis ficaram parados 2 meses, o culto muito provavelmente obteve sucesso.
3. Gere 3 a 5 eventos (Rumores, Mudanças, Consequências) que aconteceram no cenário durante esse período.

## Saída
Um relatório narrativo das "Consequências do Tempo", descrevendo como as facções se movimentaram.
Apresente os eventos ao Mestre para aprovação ou modificação.
