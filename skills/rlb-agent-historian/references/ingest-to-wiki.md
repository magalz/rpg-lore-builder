...
## Contexto
O usuário (Mestre) acaba de disponibilizar um novo documento na pasta `_bmad/memory/rlb/raw/`. 
...
2. **Atualização ou Criação:** 
   - Para entidades que já existem na pasta `_bmad/memory/rlb/wiki/`, atualize os arquivos Markdown originais adicionando os novos fatos.
...
4. **Alinhamento de Schema:** Se a pasta `_bmad/memory/rlb/schema/` possuir templates de como criar um NPC ou Local, você deve seguir essa estrutura rigorosamente.
